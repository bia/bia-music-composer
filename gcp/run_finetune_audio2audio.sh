...
for zone in "${ZONES[@]}"; do
    ...
    result=$(gcloud compute instances create $INSTANCE_NAME \
        ...
        --metadata=GCS_BUCKET_NAME=${GCS_BUCKET_NAME},GCS_BUCKET_FOLDER_PREFIX=${GCS_BUCKET_FOLDER_PREFIX},MODEL_SIZE=${MODEL_SIZE_LOWER},RUN_MODE=${RUN_MODE},NUM_EPOCHS=${NUM_EPOCHS:-3},LORA_RANK=${LORA_RANK:-16},LEARNING_RATE=${LEARNING_RATE:-5e-5},MAX_AUDIO_MS=${MAX_AUDIO_MS:-30000},DOCKER_IMAGE=${DOCKER_IMAGE},AUDIO_ENCODER_MODEL=${AUDIO_ENCODER_MODEL:-m-a-p/MERT-v1-95M},NUM_PREFIX_TOKENS=${NUM_PREFIX_TOKENS:-64},MELODIC_LOSS_WEIGHT=${MELODIC_LOSS_WEIGHT:-0.1} \
