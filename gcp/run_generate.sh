#!/bin/bash
# Launch a SPOT VM to run inference with the fine-tuned adapter and upload results to GCS.
# Usage: bash gcp/run_generate.sh
...
INSTANCE_NAME="heartmula-generate"
...
# L4 zones (same GPU used for training)
ZONES=(
...
echo "--- Creating SPOT VM: $INSTANCE_NAME (T4) ---"
CREATED_ZONE=""
for zone in "${ZONES[@]}"; do
    ...
    result=$(gcloud compute instances create $INSTANCE_NAME \
        ...
        --metadata=GCS_BUCKET_NAME=${GCS_BUCKET_NAME},GCS_BUCKET_FOLDER_PREFIX=${GCS_BUCKET_FOLDER_PREFIX},MODEL_SIZE=${MODEL_SIZE_LOWER},RUN_MODE=${RUN_MODE},NUM_OUTPUTS=${NUM_OUTPUTS:-3},MAX_AUDIO_MS=${MAX_AUDIO_MS:-30000},TEMPERATURE=${TEMPERATURE:-1.0},CFG_SCALE=${CFG_SCALE:-3.0},DOCKER_IMAGE=${DOCKER_IMAGE},INPUT_WAV=${INPUT_WAV},GCS_TAGS_FILE=${GCS_TAGS_FILE:-} \
        --metadata-from-file=startup-script=gcp/generate_startup.sh \
        2>&1) || true
