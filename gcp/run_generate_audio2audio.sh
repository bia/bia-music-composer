...
for zone in "${ZONES[@]}"; do
    ...
    result=$(gcloud compute instances create $INSTANCE_NAME \
        ...
        --metadata=GCS_BUCKET_NAME=${GCS_BUCKET_NAME},GCS_BUCKET_FOLDER_PREFIX=${GCS_BUCKET_FOLDER_PREFIX},MODEL_SIZE=${MODEL_SIZE_LOWER},RUN_MODE=${RUN_MODE},NUM_OUTPUTS=${NUM_OUTPUTS:-3},MAX_AUDIO_MS=${MAX_AUDIO_MS:-30000},TEMPERATURE=${TEMPERATURE:-1.0},CFG_SCALE=${CFG_SCALE:-3.0},DOCKER_IMAGE=${DOCKER_IMAGE},INPUT_WAV=${INPUT_WAV},GCS_TAGS_FILE=${GCS_TAGS_FILE:-},AUDIO_ENCODER_MODEL=${AUDIO_ENCODER_MODEL:-microsoft/wavlm-base},NUM_PREFIX_TOKENS=${NUM_PREFIX_TOKENS:-32} \
