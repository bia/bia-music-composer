...
#
# Internal usage only:
# The environment variable ARTIFACT_REGISTRY_PROJECT can be set to refer to any GCP project
# where the repositories/packages exist.
...
ARTIFACT_REGISTRY_PROJECT="${ARTIFACT_REGISTRY_PROJECT:-cloud-ops-agents-artifacts-dev}"

# Base URL for the Ops Agent documentation.
...
handle_debian() {
  ...
  add_repo() {
    [[ -n "${REPO_CODENAME:-}" ]] || lsb_release -v >/dev/null 2>&1 || { \
      run_with_retry apt_get_update || true
      run_with_retry apt-get -y install lsb-release
      CHANGED=1
    }
    [[ "$(dpkg -l apt-transport-https 2>&1 | grep -o '^[a-z][a-z]')" == 'ii' ]] || { \
      run_with_retry ${DRY_RUN} apt_get_update || true
    ...
    [[ "$(dpkg -l gnupg 2>&1 | grep -o '^[a-z][a-z]')" == 'ii' ]] || { \
      run_with_retry ${DRY_RUN} apt_get_update || true
      ${DRY_RUN} apt-get -y install gnupg
      CHANGED=1
    }
    ...
    if [[ -n "${ARTIFACT_REGISTRY_REGION}" ]]; then
      ...
      REPO_URL="https://${ARTIFACT_REGISTRY_REGION}-apt.pkg.dev/projects/${ARTIFACT_REGISTRY_PROJECT}"
    else
      REPO_URL="https://${REPO_HOST}/apt"
    fi
    local USE_SIGNEDBY=0
    if dpkg --compare-versions "$(dpkg-query --showformat='${Version}' --show apt)" ge 2.9.17; then
      USE_SIGNEDBY=1
    fi
    if [[ "${USE_SIGNEDBY}" == 1 ]]; then
      REPO_DATA="deb [signed-by=${KEYRING_PATH}] ${REPO_URL} ${REPO_NAME} main"
    else
      REPO_DATA="deb ${REPO_URL} ${REPO_NAME} main"
    fi
    if ! cmp -s <<<"${REPO_DATA}" - /etc/apt/sources.list.d/google-cloud-ops-agent.list; then
      ...
      add_key() {
        if [[ "${USE_SIGNEDBY}" == 1 ]]; then
          ${DRY_RUN} curl --connect-timeout 5 --silent --fail "$1" > "${KEYRING_PATH}"
        else
      ...
      add_key "https://${REPO_HOST}/apt/doc/apt-key.gpg"
    ...
    run_with_retry ${DRY_RUN} apt_get_update \
      -o Dir::Etc::sourcelist="sources.list.d/google-cloud-ops-agent.list" \
      -o Dir::Etc::sourceparts="-" || refresh_failed 'apt' "${ID}"
  }
  ...
  install_agent() {
    ...
    expected_version_installed || { \
      ...
      ${DRY_RUN} apt-get -y --allow-downgrades "${EXTRA_OPTS[@]}" install \
        "${AGENT_PACKAGE}${AGENT_VERSION}" "${ADDITIONAL_PACKAGES[@]}" || \
        fail "${AGENT_PACKAGE} ${ADDITIONAL_PACKAGES[*]} installation failed."
...
handle_rpm() {
  ...
  add_repo() {
    ...
    if [[ -n "${ARTIFACT_REGISTRY_REGION}" ]]; then
      ...
      # EL10 based distributions changed something about the kernel
      # such that the --hardware-platform flag no longer retrieves
      # a value. As a fallback, use the --machine flag, which ends
      # up giving the same value --hardware-platform used to give.
      if [[ "$BASEARCH" == "unknown"  ]]; then
        BASEARCH="$(uname --machine)"
      fi
      REPO_NAME="${REPO_NAME//\$basearch/${BASEARCH}}"
      REPO_NAME="${REPO_NAME//_/-}"  # Replace underscores with hyphens.
      REPO_URL="https://${ARTIFACT_REGISTRY_REGION}-yum.pkg.dev/projects/${ARTIFACT_REGISTRY_PROJECT}/${REPO_NAME}"
    else
...
gpgkey=https://${REPO_HOST}/yum/doc/yum-key.gpg
    ...
    run_with_retry ${DRY_RUN} yum -y \
      --disablerepo="*" --enablerepo=google-cloud-ops-agent \
      list updates || refresh_failed 'yum' "${ID}"
  }
